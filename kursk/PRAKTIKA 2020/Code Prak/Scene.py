...
class MyScene:
	...
	def show(self, ):
		...
		if (self.updateDinamic):
			for model in self.dinamicModel:
				model.addMatrixTranform(getMatrixShift(0, 0, self.carSpeed))	
		...
		# get dinamic:
		img = self.staticImg[:].copy()
		buf = self.staticBuf[:].copy()
		if (self.staticImgSun == None):
			imgSun = None
		else:
			imgSun = elf.staticImgSun[:].copy()
		if (self.staticBufSun == None):
			bufSun = None
		else:
			bufSun = self.staticBufSun[:].copy()
		...
		if self.withSadow and (self.updateDinamic or self.updateSun or 1):
	...
	def toImg(self, img_matrix):
		...
		for x in range(size['width']):
			for y in range(size['height']):
				color = img_matrix[x * size['width'] + y]
				if (color == 0):
					color = skyBlue
				else:
					#if (color != 729140):
	...
	def getBufImg(self,Model_Arr = None, img = None, buf = None):
		...
		size = self.size
		if (img == None):
			img = multiprocessing.Array('i', size['height'] * size['width'])
			#[[None for i in range(size['height'])] for j in range(size['width'])]
		if buf == None:
			buf = multiprocessing.Array('i', size['height'] * size['width'])
			#[[zBack for i in range(size['height'])] for j in range(size['width'])]
	...
	def getBufSun(self,Model_Arr = [], imgSun = None, bufSun = None):
		...
		if (imgSun == None):
			imgSun = multiprocessing.Array('i', size['height'] * size['width'] * 4)
			#[[None for i in range(size['height'] * 2)] for j in range(size['width'] * 2)]

		if (bufSun == None):
			bufSun = multiprocessing.Array('i', size['height'] * size['width'] * 4)
			#[[zBack for i in range(size['height'] * 2)] for j in range(size['width'] * 2)]
	...
	def processZbuf(self, model, buf, img = None, fl = 0):
		l = int(round(len(buf[:]) ** 0.5))

		size = {'width': l, 'height': l }
		V = model.V
		color_arr = model.polygonColor
		print("processZbuf: ", color_arr)
		for i in range(len(model.P)):
			...
			for p in point_Arr:
				if (0 < p.x and p.x < size['width'] and 0 < p.y and p.y < size['height'] ) or fl:
					if p.z > buf[p.x * size['width'] + p.y]:
						buf[p.x * size['width'] + p.y] = p.z
						if (img != None):
							img[p.x * size['width'] + self.size['height'] - p.y] = color
			'''
			processZbufParallel(img, buf, color_arr, model.P, V,  i, i+1)
				...
				if z > zBack:
					...
					if (pointSun.x > 0 and pointSun.x < size['width'] and pointSun.y > 0 and pointSun.y < size['height']) or 1:
						if (abs(pointSun.z  - bufSun[int(round(pointSun.x))][int(round(pointSun.y))]) > eps):# không nhìn thây từ mặt trời
							...
							img[x * size['width'] + self.size['height'] - y] = black.mix(img[x * size['width'] + self.size['height'] - y], 0.8)
		return img				
...
def PointZBuf(triangle = None):
	...
	for y_now in range(yMin, yMax + 1):
		...
		for x_now in range(xstart, xend, 1):
			...
			p = Point3D(x_now, y_now, z_now)
			point_Arr.append(p)
		
	return point_Arr

def processZbufParallel(img, buf, color_arr, P, V, start, end):
	for i in range(start, end):
		color = color_arr[i]
		p = P[i]
		point_Arr = PointZBuf([V[p[0]], V[p[1]], V[p[2]]])
		# 
		for p in point_Arr:
			if (0 < p.x and p.x < size['width'] and 0 < p.y and p.y < size['height'] ):
				if p.z > buf[p.x * size['width'] + p.y]:
					buf[p.x * size['width'] + p.y] = p.z
					if (img != None):
						img[p.x * size['width'] + size['height'] - p.y] = color